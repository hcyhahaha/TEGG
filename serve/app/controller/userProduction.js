...
class UserProductionController extends Controller {
    ...
    async uploadImg() {
        ...
        //上传的有作品
            ...
            var imgurl = `http://localhost:7001/upload/userProductionImg/${filename1}`
    ...
    //获取所有作品
    async productionImg() {
        console.log("99999获取所有作品");
        this.ctx.body = await this.ctx.service.userProduction.productionImg()
    }
    //图片 标题,分类,名字等等  关键字搜索
    async keydords() {
        var keydords=this.ctx.request.query.keydords
        var sql=`select * from upload where description like "%${keydords}%" or imgtitle like "%${keydords}%" or imgname like "%${keydords}%" or imgtype like "%${keydords}%"`
        this.ctx.body = await this.app.mysql.query(sql)
		// console.log(this.ctx.body)
		// this.ctx.body = "666601";
    }
