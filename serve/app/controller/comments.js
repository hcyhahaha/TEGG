...
class CommentsController extends Controller {
  ...
  async setComments() {
    ...
    if (!this.ctx.session.userid) {
      ...
      console.log("缓存的用户id", this.ctx.session.userid);

      // var sql1 = `select * from userinfo`
      var sql1 = `select * from userinfo where id = ${this.ctx.session.userid}`
      var userinfo = await this.app.mysql.query(sql1)
      console.log("前端传的字段", this.ctx.request.query);
      userinfo = userinfo[0]
      console.log("userinfo返回信息2", userinfo);
